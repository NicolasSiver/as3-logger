package im.siver.logger.controllers
{
	...
	import im.siver.logger.views.components.windows.CodeWindow;
	import im.siver.logger.views.components.windows.HelpWindow;
	import im.siver.logger.views.components.windows.StackWindow;
		...
		private function buttonClicked(event:MouseEvent):void
		{
			switch (event.target) {
				case _panel.playButton:
					_send({command:Constants.COMMAND_RESUME});
					break;
				case _panel.pauseButton:
					_send({command:Constants.COMMAND_PAUSE});
					break;
