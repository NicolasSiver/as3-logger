	...
	public final class ApplicationController extends EventDispatcher
	{
		...
		private function onInspectClicked(event:MouseEvent):void
		{
			...
			if (_panel.inspectCheckbox.selected) {
				_send({command:Constants.COMMAND_START_HIGHLIGHT});
			} else {
				_send({command:Constants.COMMAND_STOP_HIGHLIGHT});
			}
		...
		public function treeOpen(event:TreeEvent):void
		{
			...
			if (event.item.@target != null) {
				_send({command:Constants.COMMAND_GET_OBJECT, target:String(event.item.@target)});
			}
		}
		...
		public function treeClick(event:MouseEvent):void
		{
			...
			if (event.currentTarget.selectedItem != null) {
				...
				if (objType != Constants.TYPE_WARNING && objType != Constants.TYPE_STRING && objType != Constants.TYPE_BOOLEAN && objType != Constants.TYPE_NUMBER && objType != Constants.TYPE_INT && objType != Constants.TYPE_UINT && objType != Constants.TYPE_FUNCTION) {

					// Send commands
					_send({command:Constants.COMMAND_HIGHLIGHT, target:objTarget});
					_send({command:Constants.COMMAND_GET_PROPERTIES, target:objTarget});
					_send({command:Constants.COMMAND_GET_FUNCTIONS, target:objTarget});
					_send({command:Constants.COMMAND_GET_PREVIEW, target:objTarget});
				}
			}
		}
		...
		public function onRefresh(evt:ContextMenuEvent = null):void
		{
			if (_panel.tree.selectedItem != null) {
				...
				_send({command:Constants.COMMAND_GET_OBJECT, target:objTarget});
				...
				_send({command:Constants.COMMAND_BASE});
			}
		}
