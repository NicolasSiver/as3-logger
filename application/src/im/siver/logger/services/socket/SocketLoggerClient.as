package im.siver.logger.services.socket {
    ...
    public final class SocketLoggerClient implements LoggerClient {
         ...
         * Wrapper for socket
         */
        public function SocketLoggerClient(socket:Socket) {
            ...
            send(Constants.LOGGER_V1_ID, {command: Constants.COMMAND_HELLO});
        }
        ...
        public function send(id:String, data:Object):void {
            ...
            if (_socket != null) {
                ...
                // The package
                var bytes:ByteArray = new ClientData(id, data).bytes;
        ...
        private function processPackage():void {
            ...
            if (_length != 0 && _package.length == _length) {
                ...
                if (item.id != null && item.id == Constants.LOGGER_V1_ID) {
                    processData(item);
                }
        ...
        private function processData(item:ClientData):void {
            if (item.data["command"] == Constants.COMMAND_INFO) {
                ...
                // Get the roots
                send(item.id, {command: Constants.COMMAND_BASE});
