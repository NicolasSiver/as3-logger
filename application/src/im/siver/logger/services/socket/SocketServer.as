package im.siver.logger.services.socket {
    ...
    public class SocketServer {
        ...
        public static function send(uid:String, id:String, data:Object):void {
            for (var client:Object in _clients) {
                if (_clients[client] == uid) {
                    SocketMonsterClient(client).send(id, data);
                }
            }
        }
        ...
        private static function dataHandler(event:ProgressEvent):void {
            ...
            // Create a new client
            var client:LoggerClient = null;

            if (command == Constants.LOGGER_V1_ID) {
                client = new SocketLoggerClient(socket);
            } else if (command == '<hello/>\n') {
                client = new SocketMonsterClient(socket);
            }
         ...
         */
        private static function startClient(client:LoggerClient):void {
            // Connect
         ...
         * THIS IS A CALLBACK FUNCTION
         */
        private static function removeClient(client:LoggerClient):void {
            client.onData = null;
            client.onStart = null;
