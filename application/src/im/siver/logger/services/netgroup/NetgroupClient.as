package im.siver.logger.services.netgroup
{
	...
	public final class NetgroupClient implements LoggerClient
	{
			...
			switch (event.info.code) {
				...
				case "NetGroup.SendTo.Notify":
					if (event.info.from == _id) {
						var item:ClientData = new ClientData(event.info.message.id, event.info.message.data);
						if (item.id != null) {
							processData(item);
						}
					}
