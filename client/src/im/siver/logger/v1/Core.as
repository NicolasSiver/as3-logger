...
package im.siver.logger.v1
{
	...
	internal class Core
	{
		...
		// The core id
		internal static const ID:String = "im.siver.logger.core";
		...
		internal static function breakpoint(caller:*, id:String = "breakpoint"):void
		{
			...
			if (Logger.enabled && Connection.connected) {
				
				// Get the stacktrace
				var stack:XML = Utils.stackTrace();
			...
			switch(item.data["command"])
			{
				...
				case Constants.COMMAND_SET_PROPERTY:
					...
					if (obj != null) {
						try {
							...
							send({command:Constants.COMMAND_SET_PROPERTY, target:item.data["target"], value:obj[item.data["name"]]});
						} catch (e1:Error) {
							//
						}
					}
				...
				case Constants.COMMAND_GET_PREVIEW:
					...
					if (obj != null && Utils.isDisplayObject(obj)) {
						...
						if (bitmapData != null) {	
							var bytes:ByteArray = bitmapData.getPixels(new Rectangle(0, 0, bitmapData.width, bitmapData.height));
							send({command:Constants.COMMAND_GET_PREVIEW, bytes:bytes, width:bitmapData.width, height:bitmapData.height});
						}
					}
				break;
				...
				case Constants.COMMAND_PAUSE:
					Utils.pause();
					send({command:Constants.COMMAND_PAUSE});
				break;
				
				// Resume the application
				case Constants.COMMAND_RESUME:
					Utils.resume();
					send({command:Constants.COMMAND_RESUME});
				break;
